...
# poisson regression -----
poisson_df = data.frame(y = sample(seq.int(10), 100, replace = T),
                                    x1 = rnorm(100),
                                    x2 = rnorm(100))

poisson_mod = glm(y ~ x1 + x2, poisson_df, family="poisson")
coef(poisson_mod)
## coefs ----
# get analytic solution for coefficients
X_matrix = as.matrix(cbind(1, poisson_df[c("x1","x2")]))

poisson_regressions_coefs = function(params){
  # params = c(.1, .1, .1)
  ...
  back_transformed_ys = exp(predicted_ys)
  
  log_likelihoods = -back_transformed_ys + poisson_df$y * 
    log(back_transformed_ys) - log(factorial(poisson_df$y))
...
optimized_p_betas = optim(par = c(.1,.1,.1), fn = poisson_regressions_coefs)$par

# comparing betas between glm and manual
coef(poisson_mod);optimized_p_betas

## SEs -----
# deriving standard errors manually

se = sqrt(diag(solve(t(X_matrix) %*% diag(c(exp(X_matrix %*% optimized_p_betas))) %*% X_matrix)))

# compare manual SEs to lm SEs 
se;summary(poisson_mod)$coefficients[, "Std. Error"]
